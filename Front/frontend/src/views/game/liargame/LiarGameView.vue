<template>
  <div class="wrap-blue">
    ...
    <button v-show="!voted" v-for="(user, idx) in userListKey" @click="pickUser(userListKey[idx])" :key="idx">{{user}} {{userList[user]}}</button>
    <br>
...
export default {
    setup() {
        ...
        const goResult = () => {
            ...
            const votedUser = Object.keys(userList.value).find(key => userList.value[key] === maxVoted)
